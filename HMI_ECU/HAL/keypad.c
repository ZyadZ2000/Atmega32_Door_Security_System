...
uint8 KEYPAD_getPressedKey(void) {
	...
	while (1) {
		for (row = 0; row < KEYPAD_NUM_ROWS; row++) /* loop for rows */
		{
			...
			/* Clear the row output pin and set the rest pins value */
			keypad_port_value = ~(1 << (KEYPAD_FIRST_ROW_PIN_ID + row));
#else
			/* Set the row output pin and clear the rest pins value */
			keypad_port_value = (1<<(KEYPAD_FIRST_ROW_PIN_ID+row));
#endif

			GPIO_writePort(KEYPAD_ROW_PORT_ID,keypad_port_value);
